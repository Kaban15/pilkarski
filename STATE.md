...
**Aktualny etap:** 80 etapów ukończonych
...
- Strona turnieju 4-taby (Drużyny/Grupy/Drabinka/Info)
...
- Digest Card na feedzie (per rola)

### UI/Design
- Deep Charcoal palette, dual accent: violet (primary) + orange (sport), Rubik (display) + Inter (body)
- Sidebar collapsed 64 px, Top tabs role-specific, Right panel 260 px (mini kalendarz, upcoming, szybkie akcje)
...
### Testy & security
- E2E: Playwright, 31+ testów (52 pass / 1 skip po etapie 78)
- Unit: Vitest 76 testów (format, form-errors, is-club-member, file-validation, auth, tournament-logic, digest, reputation), coverage v8
...
| Etap | Data | Opis |
|------|------|------|
| 80 | 2026-04-19 | `/simplify` review pass na etap 79 — 3 agenty (reuse/quality/efficiency) w parallel. **2 actionable findings naprawione (`c6e6a50`):** (1) `tournament.ts:800-833` — 33-liniowy blok obliczający `winnerUserId` (2 DB queries: `tournamentStanding.findFirst` lub `tournamentMatch.findFirst` z FINAL phase) został martwy po usunięciu `awardPoints(winnerUserId, "tournament_win")` w etap 79. Write-only dead code → 1 mniej round-trip per tournament `complete`. (2) `dashboard-stats.tsx`: `PlayerStats`/`ClubStats` straciły 4. kartę (Ranking) w etap 79, zostały 3 karty w gridzie `lg:grid-cols-4` → pusta 4. kolumna. Adaptive `lg:grid-cols-3` gdy `stats.length===3`. **Reszta clean:** brak leftover importów, orphaned queries, martwych `useEffect`, stringly-typed leftovers ("ranking"/"points"/"gamification"). Prisma client regenerated. Net −35 linii. tsc 0, unit 76/76. |
| 79 | 2026-04-19 | Usunięcie gamifikacji (ranking, punkty, odznaki, activity heatmap) — user feedback: nie potrzebne na platformie. Usunięte: `/ranking` page + widget, `src/lib/gamification.ts`, `src/server/award-points.ts`, `src/server/trpc/routers/gamification.ts`, `src/components/activity-heatmap.tsx`, `src/lib/activity-utils.ts` i testy (gamification, award-points, activity-utils). Usunięto wywołania `awardPoints()` z 6 routerów (event, sparing, tournament, recruitment, club-post, review). Usunięto linki `/ranking` z sidebar/top-tabs/bottom-nav/command-palette/prefetch/landing-preview/feed-client/dashboard-stats. Usunięto modele Prisma `UserPoints` + `UserBadge` (migracja `20260419180000_drop_gamification` — `DROP TABLE user_badges; DROP TABLE user_points`). `ActivityHeatmap` zniknął z profili gracza/trenera/klubu (używał tych samych tabel). tsc 0, unit 76/76 (było 103), build OK. |
| 78 | 2026-04-19 | E2E helper hardening — P1+P2 z STATE etap 77 zdiagnozowane jako **jeden root cause**: `completeClubOnboarding` używał `isVisible({timeout:1500}).catch(()=>false)` (snapshot, nie wait). `clubProfile` query loaduje ~3-5s (pooler cold), banner pojawiał się za późno → helper zero-op → klub bez regionu w DB. `createQuickSparing` nie wymaga regionu, więc test szedł dalej i łapał błąd dopiero przy `DigestCard` (guard `!showOnboarding` w feed-client). **Hipoteza „React Compiler race" z etapu 77 była błędna** — Select działa poprawnie, helper nigdy nie dochodził do kliknięcia. Fix: `waitFor({state:"visible", timeout:15000})` na banner, `toBeVisible`+`toBeEnabled` przed click, `waitForResponse` na `club.update`. Assertion końcowa z `getByRole("link","Dodaj sparing")` (FAB kolizja strict mode) → `getByText("Znajdź rywala na mecz sparingowy")`. `applyToSparing` przepisany: `data-testid="sparing-apply-submit"` w `apply-form.tsx` + single click + `waitForResponse`. Unskip: `digest.spec.ts:44`, `dashboard-sections.spec.ts:60`. Wynik: **49/3→52/1**. Pozostały skip: `onboarding.spec.ts:57` (test.fixme, wymaga app fix useTransition). tsc 0, unit 103/103. |
...
src/server/trpc/routers/             — auth, club, player, coach, region, sparing, event,
                                       ...
                                       review, transfer, push, recruitment,
...
src/lib/{format,rate-limit,training-presets}.ts
...
> Sesja 2026-04-19 zamknęła etapy 75 + 76 + 77 + 78.
> **Wszystkie pre-existing E2E failures rozwiązane** (#19, #20, #22).
> Pozostał 1 skip: `onboarding.spec.ts:57` (test.fixme placeholder,
> wymaga app-level fix — useTransition na „Pomiń" re-mount).
> **Perf: 4 fixy zdeployowane + manual steps (migracja + cron) zrobione.**

### ⚠️ Przed startem nowej sesji — status deploy
- **Branch `main`:** etapy 74 + 75 + 76 + 77 + 78 pushed.
...
### Priority 1 — weryfikacja after-fix perf (etap 76)
...
| ~~20~~ | ~~E2E `dashboard-sections.spec.ts:60`~~ | ~~✅ Etap 78 — błędna hipoteza (nie shadcn Select / React Compiler, tylko `isVisible({timeout:1500})` w helperze zamiast blocking wait). Fix: `waitFor({state:"visible", timeout:15000})` + `toBeEnabled` przed click + `waitForResponse` na `club.update`. Unskipped.~~ |
...
| ~~22~~ | ~~E2E `digest.spec.ts:44` — `applyToSparing` helper modal flaky~~ | ~~✅ Etap 78 — nie było żadnego modala; helper klikał inline button 2× (drugi click w disabled pending/justApplied state). Fix: `data-testid="sparing-apply-submit"` + single click + `waitForResponse` na `sparing.applyFor`. Unskipped.~~ |
