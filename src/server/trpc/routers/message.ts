...
export const messageRouter = router({
  ...
  getClubChat: protectedProcedure.query(async ({ ctx }) => {
    ...
    if (!conversation) {
      ...
      // Add participant only if not already present (avoids upsert on every poll)
      const existing = await ctx.db.conversationParticipant.findUnique({
      ...
      if (!existing) {
        await ctx.db.conversationParticipant.create({
          data: { conversationId: conversation.id, userId: ctx.session.user.id },
        });
      }
    }
    ...
    const [messages, memberCount] = await Promise.all([
      ...
      ctx.db.clubMembership.count({ where: { clubId, status: "ACCEPTED" } }),
    ]);
  ...
  sendToClubChat: rateLimitedProcedure({ maxAttempts: 30 })
    ...
    .mutation(async ({ ctx, input }) => {
      const conversation = await ctx.db.conversation.findUnique({
        ...
        select: { clubId: true, club: { select: { userId: true } } },
      });
      if (!conversation?.clubId) throw new TRPCError({ code: "FORBIDDEN" });

      const isOwner = conversation.club?.userId === ctx.session.user.id;
      if (!isOwner) {
        const membership = await ctx.db.clubMembership.findFirst({
